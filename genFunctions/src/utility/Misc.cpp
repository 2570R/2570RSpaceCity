...
float Misc::getHeading(bool reverse) {
  ...
  //return (reverse) ? 0.5 * (inertial_sensor.rotation(vex::deg) + imu2.rotation(vex::deg)) + 180.0 : 0.5 * (inertial_sensor.rotation(vex::deg) + imu2.rotation(vex::deg));
  return (reverse) ? reduceAngle(inertial_sensor.rotation(vex::deg) + 180.0) : reduceAngle(inertial_sensor.rotation(vex::deg));
}
...
void Misc::runDrivetrain(float left, float right, bool reverse) {
  ...
  if (reverse) {
    left_chassis.spin(vex::fwd, -right, vex::voltageUnits::mV);
    right_chassis.spin(vex::fwd, -left, vex::voltageUnits::mV);
  }
